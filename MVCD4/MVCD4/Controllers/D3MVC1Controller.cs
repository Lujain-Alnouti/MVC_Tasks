...
namespace MVCD4.Controllers
{
    public class D3MVC1Controller : Controller
    {
        ...
        public ActionResult Search(string search, string searchBut)
        {
            ...
            {
                return View("Index", db.D3MVC.Where(p => p.First_Name.Contains(search)).ToList());
            }
            else if (searchBut == "LastName")
            {
                return View("Index", db.D3MVC.Where(p => p.Last_Name.Contains(search)).ToList());
            }
            else
            {
                return View("Index", db.D3MVC.Where(p => p.E_mail.Contains(search)).ToList());
            }
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,First_Name,Last_Name,E_mail,Phone,Age,Job_Title,Gender,Image,CV")] D3MVC d3MVC, HttpPostedFileBase image, HttpPostedFileBase cv)
        {
